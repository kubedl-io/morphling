...
func (k *MorphlingUIHandler) getExperimentList(namespace []string, typ JobType) ([]JobView, error) {
	jobs := make([]JobView, 0)

	el, err := k.morphlingClient.GetProfilingExperimentList(namespace...)
	if err != nil {
	...
	for _, experiment := range el.Items {
		...
		{
			experimentLastCondition, err := util.GetLastConditionTypeProfiling(&experiment)
			if err != nil {
...
func (k *MorphlingUIHandler) getAvailableNamespaces() ([]string, error) {
	var namespaces []string

	namespaceList, err := k.morphlingClient.GetNamespaceList()
	if err != nil {
		namespaces = append(namespaces, consts.DefaultMorphlingNamespace)
...
//func (k *MorphlingUIHandler) getTrialTemplatesViewList() ([]TrialTemplatesView, error) {
...
//		trialTemplatesConfigMapList, err := k.morphlingClient.GetTrialTemplates(ns)
//		if err != nil {
...
//func (k *MorphlingUIHandler) updateTrialTemplates(
//	edittedNamespace,
...
//
//	templates, err := k.morphlingClient.GetConfigMap(edittedConfigMapName, edittedNamespace)
//	if err != nil {
...
//	}
//
//	err = k.morphlingClient.UpdateConfigMap(templatesConfigMap)
//	if err != nil {
